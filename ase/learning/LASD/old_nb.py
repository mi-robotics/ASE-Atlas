...
class LASDBuilder(ASEBuilder):
    ...
    class Network(ASEBuilder.Network):
        ...
        def forward(self, obs_dict):
            ...
            noise = obs_dict.get('vae_noise', None)
            states = obs_dict.get('rnn_states', None)
            use_hidden_latents = obs_dict.get('use_hidden_latents', False)

            actor_dict = self.eval_actor(obs, ase_latents, noise=noise)
        ...
        def eval_actor(self, obs, ase_latents, use_hidden_latents=False, use_mean=False, noise=None):
            a_out = self.actor_cnn(obs)
            a_out = a_out.contiguous().view(a_out.size(0), -1)
            mu, latents, params, recon, noise = self.actor_vae(a_out, ase_latents, use_mean=use_mean, noise=noise)
